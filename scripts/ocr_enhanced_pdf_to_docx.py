#!/usr/bin/env python3
"""
OCR-Enhanced PDF to DOCX Converter for Legal Documents
======================================================

This script uses OCR to read text positioning and formatting from PDFs,
ensuring proper numbered list formatting with hanging indents.
"""
...
class OCREnhancedLegalDocumentConverter:
    """OCR-enhanced converter for legal PDF documents with perfect formatting."""
    ...
    def extract_text_with_ocr(self, page):
        ...
        mat = page.get_pixmap(matrix=fitz.Matrix(2, 2))  # Higher resolution for better OCR
        ...
        # Use Tesseract OCR with positioning
    ...
    def convert_pdf_to_docx(self, pdf_path, docx_path):
        """Convert PDF to DOCX with OCR-enhanced processing."""
        logger.info(f"Starting OCR-enhanced conversion: {pdf_path} -> {docx_path}")
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                # Detect numbered lists with OCR
                numbered_items = self.detect_numbered_list_with_ocr(filtered_blocks)
                ...
                for item in numbered_items:
                    ...
                    # Set hanging indent for numbered lists
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python ocr_enhanced_pdf_to_docx.py <input.pdf> <output.docx>", file=sys.stderr)
        sys.exit(1)
    ...
    # Convert PDF to DOCX
    converter = OCREnhancedLegalDocumentConverter()
    try:
