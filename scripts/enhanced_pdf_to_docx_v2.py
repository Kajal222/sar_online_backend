#!/usr/bin/env python3
"""
Enhanced PDF to DOCX Converter v2 for Legal Documents
====================================================

This script converts PDF legal documents (judgments, orders) to DOCX format with
intelligent formatting preservation, especially for numbered lists and alignment.
"""
...
class EnhancedLegalDocumentConverter:
    """Enhanced converter for legal PDF documents with intelligent formatting."""
    ...
    def detect_alignment(self, block, page_width=595):
        """Enhanced alignment detection based on block position and content."""
    ...
    def process_numbered_list_item(self, text):
        """Process a single numbered list item for proper formatting."""
        if not text:
            return text
            ...
            for page_num in range(len(doc)):
                ...
                for block in filtered_blocks:
                    ...
                    for item in text_items:
                        ...
                        if self.is_versus_section(processed_text):
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python enhanced_pdf_to_docx_v2.py <input.pdf> <output.docx>", file=sys.stderr)
        sys.exit(1)
    ...
    # Convert PDF to DOCX
    converter = EnhancedLegalDocumentConverter()
    try:
